...
// ─── 6-Day Home Transformation Programme ─────────────────────────────────────
interface ExerciseDef {
  ...
  restSeconds: number
  emoji: string
  isTime?: boolean   // true = reps field shows seconds
}
...
export default function WorkoutPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, user => {
      ...
      setUid(user.uid)
      // Load programme week from localStorage
  ...
  useEffect(() => {
    ...
    if (!day.isRest) setExercises(buildExerciseLogs(selectedDay))
    else setExercises([])
    setCompleted(false)
  }, [selectedDay])
    ...
    return (
      <main className="min-h-screen mesh-bg flex items-center justify-center p-5">
        <div className="max-w-sm w-full glass-strong rounded-3xl p-8 text-center space-y-5">
          <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto glow-emerald"
          ...
          {cardioLogged && <p className="text-xs text-emerald-400">✓ Cardio logged! +25 XP</p>}
          <button onClick={() => router.push('/dashboard')}
            className="w-full py-3.5 rounded-2xl font-bold text-white glow-emerald"
            style={{background:'linear-gradient(135deg,#059669,#047857)'}}>
            Back to Dashboard
          </button>
        </div>
  ...
  return (
    <main className="min-h-screen mesh-bg page-pad">
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        ...
        {!prog.isRest && programmeWeek > 1 && (
          <div className="glass rounded-2xl p-3 flex items-center gap-3">
            <span className="text-xl">📈</span>
            <div>
              <p className="text-xs font-semibold text-1">Progressive Overload Active</p>
              <p className="text-xs text-2">Target +2 reps per set vs last week (≥90% completion)</p>
            </div>
          </div>
        )}
        ...
        {!prog.isRest && exercises.map((ex, exIdx) => {
          ...
          return (
            <div key={ex.exerciseName} className="glass rounded-2xl p-4 transition-all"
              style={allDone ? {borderColor: prog.color + '60', border: `1px solid ${prog.color}60`} : {}}>
                ...
                {ex.sets.map((set, setIdx) => (
                  <div key={setIdx} className={`grid grid-cols-4 gap-2 items-center ${set.completed ? 'opacity-40' : ''}`}>
                    ...
                    <button onClick={() => completeSet(exIdx, setIdx)} disabled={set.completed}
                      className="w-9 h-9 mx-auto rounded-full text-sm font-bold transition-all flex items-center justify-center relative"
                      style={{
        ...
        <div className="text-center text-xs text-3 pb-4 flex items-center justify-center gap-1">
          ...
          SBH 12-Week Home Transformation System
        </div>
      </div>
    </main>
  )
